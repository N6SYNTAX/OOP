...
//
            ...
            var HolePath = new Path(new[] { "forward" }, Default, $"At the base of the door you notice an overgrown door with vines and tree growth partially covering it.\nWould you like to proceed through the door?", $"\n*THUMP* You slide down a long worming hole and land flat on your back in what appears to be a caveren with several paths leading away from you. You look back up at the hole you fell through, and there is no way back up", true);
            Default.Paths.Put(HolePath);
            Player.CurrentLocation = Default;

            //Cavern
            var Cavern = new Location(new[] {"cavern"}, "Cavern", $"Large Opening");
            var MineshaftPath = new Path(new[] { "forward" }, Cavern,$"Mineshaft ahead","You Walk down the path", false);
            //var TunnelPath
            //var RiverPath
            //var CliffPath
            Cavern.Paths.Put(MineshaftPath);
